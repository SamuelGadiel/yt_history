...
def load_env_config(env_path: Path | None = None) -> AppConfig:
    ...
    if not env_path.exists():
        raise ConfigError(
            f".env not found at {env_path}\nCopy .env.example to .env and configure YOUTUBE_API_KEY"
        )
    ...
    language = config_dict.get("YOUTUBE_LANGUAGE")
    region = config_dict.get("YOUTUBE_REGION")

    return AppConfig(api_key=api_key, language=language, region=region)
